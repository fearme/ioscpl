...
+(void)setUpGoogleAnalyticsService;
{
    ...
    [GAI sharedInstance].dispatchInterval = 20; // units are in seconds (the time googleSDK waits before sending analytics)
    
    [GAI sharedInstance].trackUncaughtExceptions = YES;
// Uncomment below to see google Analytics call logs
//    [[GAI sharedInstance].logger setLogLevel:kGAILogLevelVerbose];

    // following needs to be assigned even tho tracker is not used.
    [[GAI sharedInstance] trackerWithTrackingId:@"UA-69772755-5"];
}
